        ...
        <aside id="sidebar" class="lumina-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-[232px] -translate-x-full flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 transition-transform duration-300 lg:translate-x-0">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto">
                @foreach($navItems as $item)
                    ...
                    >
                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg {{ $isActive ? 'text-primary-700 dark:text-primary-300' : 'text-sidebar-muted group-hover:text-sidebar-foreground' }}">
                ...
                <div class="rounded-xl bg-sidebar-hover/50 px-3 py-3 dark:bg-white/5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-semibold text-sidebar-foreground">{{ auth()->user()->name }}</p>
                            <p class="text-[0.65rem] font-medium uppercase tracking-wider text-sidebar-muted">{{ auth()->user()->role->value }}</p>
                        </div>
                    </div>
                        ...
                        <nav class="flex items-center gap-1.5 text-sm">
                            @foreach($breadcrumbs as $crumb)
                                ...
                                @if(!empty($crumb['url']) && !$loop->last)
                                    <a href="{{ $crumb['url'] }}" class="font-medium text-muted transition hover:text-primary-700">{{ $crumb['label'] }}</a>
                                @else
