...
<body class="min-h-screen bg-slate-100 p-4 text-slate-900 sm:p-6">
    <main class="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="mb-4 flex items-center justify-between">
            ...
            <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold uppercase text-slate-700">{{ $book->status->value }}</span>
        </div>
        ...
        <div class="flex gap-4">
            ...
            >
            <div class="min-w-0">
                <h2 class="text-base font-semibold text-slate-900">{{ $book->title }}</h2>
                <p class="text-sm text-slate-600">{{ $book->author }}</p>
                <p class="mt-2 text-xs text-slate-500">ISBN: {{ $book->isbn ?? '-' }}</p>
                <p class="text-xs text-slate-500">Category: {{ $book->category->name }}</p>
            </div>
        </div>
        ...
        <div class="mt-5 rounded-xl border border-emerald-100 bg-emerald-50 p-4">
            ...
            <p class="mt-1 text-xl font-bold text-emerald-800">{{ $book->rack?->name ?? '-' }} - {{ $book->position_code ?? 'Unassigned' }}</p>
