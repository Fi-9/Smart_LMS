...
@section('content')
    ...
    <div x-data="{ viewMode: 'grid' }">
        <section class="mb-6 rounded-[2rem] border border-primary-100 bg-gradient-to-br from-white via-slate-50 to-emerald-50/50 p-6 shadow-sm dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                <div class="max-w-3xl">
                    <a href="{{ $rack->room ? route('rooms.show', $rack->room) : route('racks.index') }}" class="inline-flex items-center gap-2 rounded-2xl border border-border bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-primary-200 hover:text-primary-700 dark:bg-slate-900">
                        <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <path d="m15 18-6-6 6-6"/>
                        </svg>
                        {{ $rack->room ? 'Kembali ke ' . $rack->room->name : 'Kembali ke Peta' }}
                    </a>

                    <p class="mt-4 text-xs font-semibold uppercase tracking-[0.28em] text-gray-400">Library Map / {{ $rack->name }}</p>
                    <div class="mt-3 flex flex-wrap items-center gap-3">
                        <h1 class="text-3xl font-black tracking-tight text-gray-900">Struktur Internal Rack {{ $rack->name }}</h1>
                        <span class="rounded-2xl bg-emerald-100 px-3 py-1 text-sm font-black uppercase tracking-[0.2em] text-emerald-700">{{ $gridSystemLabel }}</span>
                    </div>
                ...
                <div class="grid gap-3 sm:grid-cols-3">
                    <div class="rounded-2xl border border-white/70 bg-white/80 px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
                        ...
                        <p class="mt-2 text-2xl font-black text-gray-900">{{ $availableSlots }}</p>
                    </div>
                    <div class="rounded-2xl border border-white/70 bg-white/80 px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
                        <p class="text-[0.7rem] font-semibold uppercase tracking-[0.24em] text-gray-400">Terisi</p>
                        <p class="mt-2 text-2xl font-black text-gray-900">{{ $filledSlots }}</p>
                    </div>
                    <div class="rounded-2xl border border-white/70 bg-white/80 px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
                        <p class="text-[0.7rem] font-semibold uppercase tracking-[0.24em] text-gray-400">Kapasitas</p>
                        <p class="mt-2 text-sm font-black uppercase tracking-[0.18em] text-emerald-700">{{ $rack->capacity_per_slot ?? 1 }}/slot</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a
                        href="{{ route('qr.print', ['rack_id' => $rack->id, 'layout' => 'tj103']) }}"
                        target="_blank"
        ...
        <div x-show="viewMode === 'grid'" x-transition>
            <section class="mb-6 rounded-[2rem] border border-border bg-surface p-6 shadow-sm">
                ...
                <div id="column-category-row" class="hidden mb-3 grid gap-4 overflow-x-auto pb-1" style="grid-template-columns: repeat({{ $rack->columns }}, minmax(7rem, 1fr));">
                    @for($col = 1; $col <= $rack->columns; $col++)
                        @php $catId = $rack->column_categories[(string)$col] ?? null; @endphp
                        <div class="relative" x-data="{ open: false }">
                            <button
                                ...
                                id="col-cat-btn-{{ $col }}"
                            >
                                <span id="col-cat-label-{{ $col }}">{{ $catId ? ($categories->firstWhere('id', $catId)?->name ?? 'Kolom '.$col) : 'Kolom '.$col }}</span>
                            ...
                            <div
                                ...
                                style="display: none;"
                            >
                                <button type="button" @click="setColumnCategory({{ $col }}, null); open = false" class="w-full px-3 py-2 text-left text-xs text-gray-500 hover:bg-gray-50 transition">— Tanpa Kategori</button>
                                @foreach($categories as $cat)
                                    <button type="button" @click="setColumnCategory({{ $col }}, {{ $cat->id }}); open = false" class="w-full px-3 py-2 text-left text-xs text-gray-700 hover:bg-primary-50 hover:text-primary-700 transition {{ $catId == $cat->id ? 'bg-primary-50 font-bold text-primary-700' : '' }}">{{ $cat->name }}</button>
                                @endforeach
                            </div>
                        </div>
                    @endfor
                ...
                <div id="rack-grid" class="grid gap-4 overflow-x-auto pb-1" style="grid-template-columns: repeat({{ $rack->columns }}, minmax(8rem, 1fr));">
                    @foreach($grid as $cell)
                        ...
                        <button
                            type="button"
                            title="{{ $cell['count'] > 0 ? ($cellTitles ?: 'Detail slot') : 'Klik untuk assign buku' }}"
                            data-grid-cell
                            ...
                            @if($canAssign) data-available-target @endif
                            @if($cell['count'] > 0) data-filled-cell @else data-empty-slot @endif
                            class="rack-slot-card {{ $cell['count'] > 0 ? 'is-filled' : '' }}"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <span class="rack-slot-label">SLOT {{ $cell['code'] }}</span>
                                <span class="rack-slot-meta">{{ $cell['count'] }}/{{ $cell['capacity'] }}</span>
                            </div>
                            ...
                            <div class="mt-3 flex h-16 items-end justify-center gap-1.5">
                                @for($barIndex = 0; $barIndex < 5; $barIndex++)
                                    ...
                                    @endphp
                                    <span class="slot-book-bar" style="height: {{ $height }}px; background-color: {{ $barColor }};"></span>
                                @endfor
                            </div>

                            <div class="mt-5 flex items-center justify-between gap-3">
                                <span class="rack-slot-caption">{{ $footerLabel }}</span>
                                <span class="text-[0.7rem] font-bold uppercase tracking-[0.2em] text-gray-400">{{ $cell['count'] > 0 ? 'Management' : 'Available' }}</span>
                            </div>
                        </button>
                    @endforeach
                </div>
    ...
    <div x-show="viewMode === 'manual'" x-cloak x-transition>
        <x-card class="mb-6 shadow-md">
            <div class="mb-4">
                <h2 class="text-base font-bold text-gray-900">📝 Manual Book Entry — {{ $rack->name }}</h2>
            ...
            <form method="POST" action="{{ route('books.import.manual') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" class="form-input bg-gray-50 cursor-not-allowed" value="{{ $rack->name }}" disabled>
                    </div>
                    <div>
                ...
                <x-button type="submit" variant="success">Simpan Buku ke {{ $rack->name }}</x-button>
            </form>
        </x-card>
    </div> {{-- end Manual Input View --}}

    </div> {{-- end x-data viewMode --}}
    ...
    {{-- Book Detail Popup --}}
    {{-- Book Info Popup removed — replaced by inline slot-driven table --}}
    ...
    <script id="rack-books-data" type="application/json">
        {!! json_encode($rack->books->map(fn($b) => ['id' => $b->id, 'title' => $b->title, 'author' => $b->author, 'status' => $b->status?->value ?? (string) $b->status, 'position_code' => $b->position_code])->values()->all()) !!}
    </script>
    ...
    <script>
        (() => {
            ...
            const bookDetailBaseUrl = '{{ url('/books') }}';
            const bookManageBaseUrl = '{{ route('books.index') }}';
            const qrPrintBaseUrl = '{{ route('qr.print') }}';
            ...
            window.setColumnCategory = async (column, categoryId) => {
                const url = '{{ route("racks.set-column-category", $rack) }}';
            ...
            const isbnLookupUrl = '{{ route("books.import.isbn-lookup") }}';
